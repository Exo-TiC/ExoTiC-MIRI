...
class CustomLinearityOddEvenStep(Step):
    ...
    def process(self, input):
        ...
        with datamodels.open(input) as input_model:
            ...
            linearity_model_odd = self.compute_linearity_model(input_model, odd_or_even_rows="odd")
            linearity_model_even = self.compute_linearity_model(input_model, odd_or_even_rows="even")

            linearity_model = linearity_model_odd.copy()
            linearity_model.coeffs[:, 1::2, :] = linearity_model_even.coeffs[:, 1::2, :]

            linearity_model.meta.ref_file = input_model.meta.ref_file

        return linearity_model

    def compute_linearity_model(self, input_model, odd_or_even_rows="odd"):
        if odd_or_even_rows == "odd":
            row_idx_su = int(np.ceil(self.row_idx_start_used) // 2 * 2)
        else:
            row_idx_su = int(np.ceil(self.row_idx_start_used) // 2 * 2 + 1)

        groups_all = np.arange(self.group_idx_start_derive,
                               self.group_idx_end_derive)  # Exclude grps beyond help, e.g., final.
        groups_fit = np.arange(self.group_idx_start_fit, self.group_idx_end_fit)
        rows = (row_idx_su, self.row_idx_end_used)
        ...
        for amp_idx, amp_col in zip(amplifier_idxs, amplifier_cols):
            ...
            ramps_all = input_model.data[
                        :, groups_all, rows[0]:rows[1]:2, amp_col] \
                .reshape(groups_all.shape[0], -1)
            ramps_fit = input_model.data[
                        :, groups_fit, rows[0]:rows[1]:2, amp_col] \
